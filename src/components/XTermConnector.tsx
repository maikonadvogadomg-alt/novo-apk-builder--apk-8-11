/**
 * XTermConnector â Terminal xterm.js com conexÃ£o WebSocket configurÃ¡vel
...
function statusLabel(s: Status) {
  if (s === "connected")    return "â Conectado";
  if (s === "connecting")   return "â³ Conectando...";
  if (s === "disconnected") return "ð´ Desconectado";
  if (s === "error")        return "â Erro";
  return "â¬ Aguardando";
}
    ...
    Promise.all([
      ...
      const connect = () => {
        ...
        if (reconnCountRef.current === 0) {
          term.writeln(`\x1b[90m[SK Terminal â Conectando a ${wsUrl}...]\x1b[0m\r\n`);
        } else {
        ...
        try {
          ...
          setStatus("error");
          setErrMsg(`URL invÃ¡lida: ${e.message}`);
          term.writeln(`\r\n\x1b[31m[Erro: URL invÃ¡lida â ${e.message}]\x1b[0m\r\n`);
          return;
        }
        ...
        const connTimeout = setTimeout(() => {
          if (ws.readyState !== WebSocket.OPEN) {
            ...
            if (!mountRef.current) return;
            setStatus("error");
            setErrMsg("Timeout â servidor nÃ£o respondeu em 10s");
            term.writeln(`\r\n\x1b[31m[Timeout â verifique se o servidor estÃ¡ ativo]\x1b[0m\r\n`);
          }
        }, 10000);
        ...
        ws.onopen = () => {
          ...
          setReconnect(n => n + 1);
          term.writeln(`\x1b[32m[Conectado â â ${wsUrl}]\x1b[0m`);
          term.writeln(`\x1b[90m[Timeout: 19 minutos Â· ReconexÃ£o automÃ¡tica: ${MAX_RECONNECT}x]\x1b[0m\r\n`);

          // Keepalive a cada 20s para nÃ£o cair por inatividade
        ...
        ws.onclose = () => {
          ...
          if (reconnCountRef.current < MAX_RECONNECT) {
            ...
            term.writeln(`\x1b[33m[MÃ¡ximo de tentativas. Clique em âº Reconectar.]\x1b[0m\r\n`);
          }
        };
        ...
        ws.onerror = () => {
          ...
          if (!mountRef.current) return;
          setStatus("error");
          setErrMsg("NÃ£o foi possÃ­vel conectar. Verifique o endereÃ§o e se o servidor estÃ¡ ativo.");
          term.writeln(`\r\n\x1b[31m[Erro de conexÃ£o â verifique o endereÃ§o WebSocket]\x1b[0m\r\n`);
